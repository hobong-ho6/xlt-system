    ...
    try:
        ...
        if not path.exists():
            ...
            try:
                ...
                return {'status': 'error', 'error': f'디렉토리 생성 실패: {e}'}
        ...
        try:
            ...
            return {'status': 'error', 'error': f'쓰기 권한 없음: {e}'}

    except Exception as e:
        return {'status': 'error', 'error': f'경로 오류: {e}'}
...
@app.route('/api/settings/test-directory', methods=['POST'])
def api_test_directory():
    """디렉토리 권한 테스트"""
    try:
        data = request.get_json()
        ...
        if not directory:
            return jsonify({
                'status': 'error',
                'error': '디렉토리 경로를 입력해주세요.'
            }), 400

        result = test_directory_access(directory)
        return jsonify(result)

    except Exception as e:
        return jsonify({
            'status': 'error',
            'error': f'디렉토리 테스트 실패: {str(e)}'
        }), 500
...
def api_save_settings():
    ...
    try:
        ...
        if figma_token:  # None이 아닌 경우만 저장 (기존 토큰 유지하려면 None 전달)
            ...
            with open(figma_config_path, 'w', encoding='utf-8') as f:
                json.dump(figma_config, f, indent=2)
...
def translate_selected():
    ...
    try:
        ...
        print("=" * 80)
        print("🔍 /translate-selected 요청 받음 (개별 XLT Key)")
        print("=" * 80)
        print(f"Session ID: {session_id}")
        print(f"Selected indexes: {selected_indexes}")
        print(f"Final texts 개수: {len(final_texts)}")
        print(f"XLT Keys 개수: {len(xlt_keys)}")
        for i, (idx, text, key) in enumerate(zip(selected_indexes, final_texts, xlt_keys)):
            print(f"  [{i}] index={idx}, key='{key}', text='{text[:50]}...'")
        print("=" * 80)
        ...
        try:
            print("=" * 80)
            print(f"🔄 번역 시작: {len(selected_texts)}개 텍스트, 모드: {translation_mode}")
            print(f"   📖 guide.md 용어집: {len(terminology)}개 용어 로드됨")
            print(f"   대상 언어: {target_languages}")
            print(f"   샘플 텍스트: {selected_texts[0] if selected_texts else 'None'}")
            print(f"   번역기 타입: {type(pipeline.translator).__name__}")
            print("=" * 80)
            ...
            # 번역 시작 로그 추가
            log_entry = {
                'message': f'🔄 번역 시작: {len(selected_texts)}개 텍스트 → {len(target_languages)}개 언어',
            ...
            for i, lang in enumerate(target_languages):
                ...
                log_entry = {
                    'message': f'   {i+1}/{len(target_languages)} {lang_names.get(lang, lang)} 번역 대기 중...',
                    'timestamp': datetime.now().strftime("%H:%M:%S"),
                    'type': 'info'
                }
                session_logs[session_id].append(log_entry)
...
def create_excel_with_custom_keys(translations, xlt_keys):
    ...
    try:
        ...
        # sampleformat.xlsx 템플릿 파일 경로
        template_path = '/Users/user/Documents/XLTTT/Sample/sampleformat.xlsx'
...
if __name__ == '__main__':
    print("🚀 XLT System v3.0 - 피그마 전용 번역 시스템")
    print("=" * 60)
    ...
    if setup_status['all_configured']:
        ...
        print(f"🌐 웹 서버 포트: {server_port}")
        print(f"🌐 접속 URL: http://localhost:{server_port}")
        print("🎯 특징: 피그마 전용 + Google 번역 + guide.md 기반 번역")
        print("=" * 60)
