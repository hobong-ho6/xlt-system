...
# XLT System v3.1 완전 자동화 설치 스크립트 v2.0
# "원클릭 완전 자동화" - 실패 불가능한 설치 시스템
...
install_packages_compatibility() {
    ...
    # rumps (macOS 전용 트레이, 선택사항) - pyobjc 9.x 사용
    info_message "⏳ rumps 설치 중 (macOS 네이티브 트레이)..."
    if ! python3 -c "import rumps" 2>/dev/null; then
        run_with_timeout 120 python3 -m pip install --user 'pyobjc-core>=9.0,<10.0' 'pyobjc-framework-Cocoa>=9.0,<10.0' rumps 2>/dev/null && {
            success_message "rumps 설치 완료"
        } || {
            warning_message "rumps 설치 실패 - 트레이 기능 제한됨 (웹 서버는 정상 작동)"
        }
    else
...
comprehensive_verification() {
    progress_bar 7 $TOTAL_STEPS "종합적 시스템 검증 중..."
...
main() {
    # 시작 메시지
    echo ""
    echo -e "${BLUE}🚀 XLT System v3.1 완전 자동화 설치 시작${NC}"
    echo -e "${BLUE}======================================================${NC}"
    echo -e "${CYAN}💡 '원클릭 완전 자동화' - 실패 불가능한 설치 시스템${NC}"
    echo ""
    ...
    # 8단계: 자가 치유 바로가기 생성
    progress_bar 8 $TOTAL_STEPS "자가 치유 바로가기 생성 중..."
    create_independent_shortcut
    success_message "자가 치유 바로가기 생성 완료"

    # 9단계: 자가 치유 시스템 활성화
    progress_bar 9 $TOTAL_STEPS "자가 치유 시스템 활성화 중..."
    activate_self_healing_system
    success_message "자가 치유 시스템 활성화됨"

    # 10단계: 완료
    progress_bar 10 $TOTAL_STEPS "설치 완료!"
    ...
    echo ""
    echo -e "${GREEN}🎉 XLT System v3.1 설치 완료!${NC}"
