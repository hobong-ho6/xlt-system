...
class XLTUpdater:
    ...
    def get_current_version(self) -> Optional[Dict[str, str]]:
        """현재 로컬 버전 정보 조회 (Git 없는 환경 지원)"""
        # 1단계: version.json 파일 우선 시도 (ZIP 설치 환경)
        try:
            ...
            if version_file.exists():
                ...
                return {
                    ...
                    'short_hash': version_data.get('commit_hash', 'unknown')[:7],
                    'message': version_data.get('commit_message', 'Version from ZIP install'),
                    'date': version_data.get('build_date', datetime.now().isoformat()),
                    'branch': 'main',
                    'version': version_data.get('version', '3.1.0'),
                    'source': 'version_file'
                }
        except Exception as e:
            print(f"⚠️ version.json 읽기 실패: {str(e)}")

        # 2단계: Git 저장소 시도 (개발 환경)
        try:
            os.chdir(self.project_root)

            # Git 저장소 존재 확인
            if not (self.project_root / ".git").exists():
                print("⚠️ Git 저장소가 아닙니다. ZIP 설치 환경으로 추정됩니다.")
                return {
                    'hash': 'zip-install',
                    'short_hash': 'zip-inst',
                    'message': 'Installed from ZIP archive',
    ...
    def check_for_updates(self) -> Dict[str, any]:
        ...
        if current.get('source') == 'version_file' or current.get('source') == 'zip_install':
            ...
            try:
                ...
                # 원격이 24시간 이상 최신이면 업데이트 가능
                time_diff = (remote_date - current_date).total_seconds()
                update_available = time_diff > 86400  # 24시간 = 86400초

            except Exception as e:
                print(f"⚠️ 날짜 비교 실패, 해시로 대체: {str(e)}")
