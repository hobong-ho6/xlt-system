...
def api_terminology():
    """용어집 데이터 API (JSON 형태로 제공)"""
    try:
        # terminology_data.json 파일에서 데이터 로드
        terminology_file = os.path.join(os.path.dirname(__file__), 'terminology_data.json')

        if not os.path.exists(terminology_file):
            return jsonify({
                'status': 'error',
                'message': '용어집 파일을 찾을 수 없습니다',
                'file_path': terminology_file
            }), 404
        ...
        # 통계 정보 추가
        stats = {
            'total_terms': len(terminology_data.get('terminology', {})),
            'total_exceptions': len(terminology_data.get('exceptions', [])),
            'languages': terminology_data.get('metadata', {}).get('languages', []),
            'version': terminology_data.get('metadata', {}).get('version', 'unknown'),
            'last_updated': terminology_data.get('metadata', {}).get('last_updated', 'unknown')
        }
        ...
        app.logger.error(f"용어집 API 오류: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'용어집 로드 실패: {str(e)}'
        }), 500

@app.route('/api/terminology/claude-format')
def api_terminology_claude_format():
    """Claude 프롬프트용 형식으로 변환된 용어집 제공"""
    try:
        # terminology_data.json 파일에서 데이터 로드
        terminology_file = os.path.join(os.path.dirname(__file__), 'terminology_data.json')
        ...
        # Claude 프롬프트 형식으로 변환
        terminology_dict = terminology_data.get('terminology', {})

        # 최대 50개로 제한 (기존 시스템과 동일)
        limit = min(50, len(terminology_dict))
        ...
        return jsonify({
            ...
            'stats': {
                ...
                'total_available': len(terminology_dict),
                'limit_applied': limit,
        ...
        app.logger.error(f"Claude 형식 용어집 API 오류: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'Claude 형식 변환 실패: {str(e)}'
        }), 500

@app.route('/api/terminology/test-sheets')
def api_terminology_test_sheets():
    """Google Sheets 용어집 시스템 테스트 API"""
    try:
        # XLT 모듈 import
        from xlt.core.config import XLTConfig
        from xlt.terminology import GoogleSheetsTerminology, TerminologyCache
        ...
        test_results = {
            'config_status': {
                ...
                'has_credentials': bool(config.get_google_credentials()),
                'cache_ttl': config.terminology_cache_ttl
            },
            'cache_status': {},
        ...
        try:
            ...
            test_results['cache_status'] = {
                'available': True,
                'directory': cache_stats['cache_directory'],
                'terminology_cache_exists': cache_stats['terminology_cache_exists'],
                'terminology_cache_valid': cache_stats['terminology_cache_valid']
            }
        except Exception as e:
            test_results['cache_status'] = {
                'available': False,
        ...
        # Google Sheets 시스템 테스트
        try:
            terminology_system = GoogleSheetsTerminology(config)
            system_status = terminology_system.get_system_status()

            test_results['google_sheets_status'] = {
                'available': system_status['available'],
                'api_connection': system_status.get('api_connection', False),
                'sheet_id': system_status.get('sheet_id'),
                'available_sheets': system_status.get('available_sheets', [])
            }

            # 실제 데이터 로드 테스트 (가능한 경우)
            if system_status['available']:
                try:
                    terminology_data = terminology_system.load_terminology()
                    test_results['integration_test'] = {
                        'load_success': True,
                        'terms_count': len(terminology_data) if terminology_data else 0,
                        'sample_terms': list(terminology_data.keys())[:3] if terminology_data else []
                    }
                except Exception as e:
                    test_results['integration_test'] = {
                        'load_success': False,
                ...
                test_results['integration_test'] = {
                    'load_success': False,
                    'message': 'Google Sheets 시스템이 비활성화되어 있습니다'
                }
        ...
        # ClaudeTranslator 통합 테스트
        try:
            from xlt.translation.claude_translator import ClaudeTranslator
            translator = ClaudeTranslator(config)
            terminology_result = translator._load_guide_terminology()

            test_results['claude_integration'] = {
                'success': True,
                'terminology_length': len(terminology_result),
                'sample_preview': terminology_result[:200] + '...' if len(terminology_result) > 200 else terminology_result
            }
        except Exception as e:
            test_results['claude_integration'] = {
                'success': False,
        ...
        return jsonify({
            ...
            'summary': {
                'google_sheets_ready': test_results['google_sheets_status'].get('available', False),
                'cache_working': test_results['cache_status'].get('available', False),
                'claude_integration_working': test_results.get('claude_integration', {}).get('success', False)
            }
        })

    except Exception as e:
        app.logger.error(f"Google Sheets 테스트 API 오류: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'Google Sheets 시스템 테스트 실패: {str(e)}'
        }), 500
