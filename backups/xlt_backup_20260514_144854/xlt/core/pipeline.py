...
class XLTPipeline:
    ...
    def __init__(self, config: Optional[XLTConfig] = None):
        ...
        self.claude_translator = None  # v4.0: Claude 번역기 추가
    ...
    def initialize(self):
        ...
        try:
            ...
            # Claude 번역기 초기화 (XLT System v4.0 - Claude Code 통합)
            try:
                from ..translation.claude_translator import ClaudeTranslator
                self.claude_translator = ClaudeTranslator(self.config)
                print("✅ Claude 번역기 준비 완료 (Claude CLI 기반)")
            except Exception as e:
                print(f"⚠️ Claude 번역기 비활성화: {str(e)}")
                self.claude_translator = None
